...
Algorithms=['LR','RF','knn','SVM','lightGBM', 'MLPClassifier', 'XGBoost']  #['LR','RF','knn','SVM','voting','lightGBM', 'MLPClassifier', 'XGBoost']
# Algorithms=['XGBoost'] 
...
Full_features = ['sex', 'age','BMI', 'smoke', 'solitude', 'fami_history',
                 'SBP', 'DBP', 'HR', 'SpO2', 'RR',
                 'CAT score', 'mMRC score', 'deterioration_times',
                 'his_asthma', 'his_TB', 'his_HT', 'his_DM', 'his_CVD','his_liver',
                 'Pre-BD-a.FEV1',  'Post-BD-a.FEV1', 'Post-BD-c.FEV1/FVC',
                 'LABA', 'LAMA', 'ICS']
...
#    plt.title('Random forest-Feature importance'+'('+ outcome +')')
    ...
    plt.xticks(np.arange(0.00, 0.2, 0.05))
...
#X_train, X_test, y_train, y_test = train_test_split(X.values, y.values,test_size=0.3, stratify=y,random_state=19)
    X_train, X_test, y_train, y_test = train_test_split(X.values, y.values,test_size=Test_split_test_Size, stratify=y, 
                                                        random_state=33)
    if if_oversampling:
        sm = SMOTE(random_state=33) #random_state=7
...
def BuildModel2(algorithms, X_train, X_test, y_train_onehot, y_test, y_test_onehot, outcome):
    y_pred_log= y_pred_rnd= y_pred_svm=y_pred_knn =y_pred_lgb= y_pred_mlp=y_pred_xgb=[]
    y_pred_log_proba= y_pred_rnd_proba= y_pred_svm_proba=y_pred_knn_proba =y_pred_lgb_proba= y_pred_mlp_proba=y_pred_xgb_proba=[]
    ...
    for alg in algorithms:
        if alg=='LR':
            ...
            LR_pipeline = Pipeline([('scaler', StandardScaler()), ('LR', LogisticRegression(max_iter=5000))])
            LR_parameters={'LR__C': [1], 'LR__max_iter': [100], 'LR__penalty': ['l2']}
...
#             LR_parameters={'penalty': ['l1', 'l2'],'C':[0.001,.009,0.01,.09,1,1.2,5,15,30,100],'max_iter':[10,30,50,100,1000]}
            ...
            if IF_LOG_NORMALIZED==True:
                PlotFeatureImportantLog(Full_features,"Logistic Regression",log_clf,X_train_n,y_train, outcome,True)                                     
            else:
        ...
        if alg=='RF':
            ...
            rfc=RandomForestClassifier(random_state=33) #random_state=42
        ...
        if alg=='knn':
            ...
            y_pred_knn, y_pred_knn_proba, fpr_knn,tpr_knn, roc_auc_knn, Performance_knn = GetModelPerformance('KNN', knn_clf, X_train, y_train, X_test, y_test)  # 未標準化
            confusion = metrics.confusion_matrix(y_test, y_pred_knn)
            np.set_printoptions(precision=2)
            plot_confusion_matrix(confusion,range(0,2),title='Confusion matrix_KNN'+'('+ outcome +')')                                                  
        if alg=='SVM':
        ...
        if alg=='lightGBM':
            lgb_parameters = {'learning_rate': [0.001],'num_iterations': [500]}
...
#            PlotFeatureImportantLgb(Full_features,"lightGBM",lgb_clf,X_train,y_train)
        ...
        if alg=='MLPClassifier': #(90,50),
            mlp_parameters = {'hidden_layer_sizes':[(100)],
                              'max_iter':[30],'learning_rate': ['constant'],
                              'learning_rate_init':[0.0001],'shuffle':[True]}
        ...
        if alg=='XGBoost':
#             xgb = XGBClassifier()             
            xgb = XGBClassifier(objective='binary:logistic',learning_rate=0.01,tree_method='exact',
                                scale_pos_weight=4,gamma=0.2,max_delta_step=15,colsample_bytree=0.25) #調參數 ,scale_pos_weight=4
...
#    plt.title('ROC curve'+'('+ outcome +')')
    ...
    report = pd.DataFrame((pd.concat([Performance_LR, Performance_RF, Performance_knn,Performance_svm,
                                          Performance_lgb, Performance_mlp, Performance_xgb], axis = 1))).T.round(decimals = 3)
    report_time = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
    report.to_csv(performance_path+ 'Model Performance_'+outcome +'_'+report_time+'.csv', index=0, encoding='utf_8_sig')
    
    return y_pred_log_proba, y_pred_rnd_proba, y_pred_svm_proba, y_pred_knn_proba, y_pred_lgb_proba, y_pred_mlp_proba,y_pred_xgb_proba # all probability
...
X_train, X_test, y_train, y_train_onehot, y_test, y_test_onehot, dynamic_class_weight= DataPreprocessing('outcome_6mon', IF_MM_SMOTE)
for i in range(1):
    print('第',i,'次建模(20).......................')
    y_pred_log_proba, y_pred_rnd_proba, y_pred_svm_proba, y_pred_knn_proba,y_pred_lgb_proba, y_pred_mlp_proba, y_pred_xgb_proba = BuildModel2(algs,  X_train, X_test, y_train_onehot, y_test, y_test_onehot,'outcome_6mon') #in_hosp_mortality
...
New_threshold, New_AUC, New_sensitivity, New_specificity = BestThresholdPerformance(y_test, y_pred_rnd_proba[:, 1],1, 0.5, 0.9)
print("New_threshold=",New_threshold, ", New_AUC=",New_AUC, ", New_sensitivity=,", New_sensitivity, ", New_specificity=",New_specificity)
...
New_threshold, New_AUC, New_sensitivity, New_specificity = BestThresholdPerformance(y_test, y_pred_xgb_proba[:, 1],1, 0.5, 0.9)
print("New_threshold=",New_threshold, ", New_AUC=",New_AUC, ", New_sensitivity=,", New_sensitivity, ", New_specificity=",New_specificity)
...
New_threshold, New_AUC, New_sensitivity, New_specificity = BestThresholdPerformance(y_test, y_pred_lgb_proba[:, 1],1, 0.5, 0.8)
print("New_threshold=",New_threshold, ", New_AUC=",New_AUC, ", New_sensitivity=,", New_sensitivity, ", New_specificity=",New_specificity)
...
# New_threshold, New_AUC, New_sensitivity, New_specificity = BestThresholdPerformance(y_test, y_pred_mlp_proba[:, 1],1, 0.5, 0.74)
# print("New_threshold=",New_threshold, ", New_AUC=",New_AUC, ", New_sensitivity=,", New_sensitivity, ", New_specificity=",New_specificity)
